...
[界面预览](#界面预览) · [为什么选择](#为什么选择-codelf) · [它能做什么](#它能做什么) · [长期记忆](#长期记忆) · [插件系统](#插件系统) · [技术栈](#技术栈) · [快速开始](#快速开始) · [打包构建](#打包构建) · [官网](https://codelf.top) · [联系我](#联系我)
...
- **会越用越懂你的长期记忆**。受人脑海马体启发的双轨记忆系统：和你聊过的事会沉淀成记忆，下次提起相关话题时自动"想起来"——记住你的偏好、项目约定、上个月做过的决策。记忆会随时间自然衰减、被再次唤起时又强化，还能在记忆之间建立联想，可视化成图谱随时查看、编辑、删除。全程本地存储，不上传。
...
市面上的 Agent 记忆大多是"存一段文本、用时检索"的简单 RAG。Codelf 借鉴人脑**海马体**的记忆机制，做了一套会自我演化的双轨长期记忆系统，让 AI 真正"记得住、想得起、会遗忘"。

### 双轨记忆

- **情景记忆（快学）**：和你的每次对话中提取的事实、偏好、决策，带向量、显著度与时间戳存入本地数据库，类似海马体对单次事件的快速编码。
- **语义记忆（慢学）**：高价值的情景记忆会在"睡眠巩固"过程中被提炼、沉淀进每个项目的 `MEMORY.md`，成为稳定的项目知识，类似大脑皮层的长期知识。

### 像人脑一样工作

- **主动联想召回**：每轮根据你的输入做语义检索，自动唤起相关往事，并沿"联想边"扩散带出关联记忆——你提一句，它就能想起很久以前相关的事。
- **遗忘与强化**：记忆强度随时间指数衰减（会"淡忘"），被再次唤起时又被强化、半衰期延长（间隔重复），久未触碰的转入休眠 / 归档。
- **冲突消解**：对同一事实的新旧矛盾记忆（如先说"男"后说"女"），按时间以最新为准，旧记忆自动让位。
- **代码空间锚定**：记忆可锚定到具体文件 / 符号，在 IDE 里回到相关代码时优先唤起，让记忆带上"在哪发生"的上下文。
- **工作区隔离**：记忆与工作区绑定，项目间互不串扰；身份 / 偏好等才作为全局记忆跨项目共享。

### 看得见、管得了

- **记忆图谱**：「设置 → 记忆 → 打开记忆库 / 图谱」可视化所有记忆与它们的联想关系，节点大小代表活跃度、颜色代表类型、半透明代表休眠。可拖动、缩放、平移。
- **查看 / 编辑 / 删除**：点击任意节点查看详情，直接编辑正文（自动重建检索向量）或删除，记忆库完全由你掌控。
- **缓存友好**：所有动态记忆只注入到消息尾部，不破坏提示词静态前缀，**对上下文缓存命中率零影响**——省钱与记忆兼得。

> 全程本地存储（better-sqlite3 + sqlite-vec + 本地中文向量模型），记忆不上传、不联网。可在「设置 → 记忆」里分别开关「新会话注入项目记忆」与「自动联想召回」。
...
| 长期记忆 | better-sqlite3、sqlite-vec（情景记忆向量库）、bge-small-zh-v1.5（本地中文向量）、react-force-graph-2d + d3-force（记忆图谱可视化） |
