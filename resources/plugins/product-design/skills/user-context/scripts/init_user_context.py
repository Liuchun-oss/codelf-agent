...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Create $CODELF_HOME/state/plugins/product-design/user-context.md "
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Codelf home directory. Defaults to $CODELF_HOME or ~/.codelf.",
    )
    parser.add_argument(
        "--state-dir",
        type=Path,
...
def resolve_state_dir(codelf_home: Path | None, state_dir: Path | None) -> Path:
    if state_dir is not None:
        return state_dir.expanduser().resolve()
    home = codelf_home or Path(os.environ.get("CODELF_HOME", "~/.codelf"))
    return (home.expanduser() / PLUGIN_STATE_DIR).resolve()


def main() -> int:
    args = parse_args()
    state_dir = resolve_state_dir(args.codelf_home, args.state_dir)
