...
- `$get-context` can run in question mode or playback mode. If required product, visual, or interactivity details are missing, ask for them. If those details are already present, play back the brief in a pithy format before moving to the next workflow. In question mode, prefer the `AskUserQuestion` tool (structured multiple-choice) over free-form prose questions.
